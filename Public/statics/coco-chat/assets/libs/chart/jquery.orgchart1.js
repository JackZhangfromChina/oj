...
(function(factory) {
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + '>')
      ...
      .append(typeof opts.nodeContent !== 'undefined' ? '<div class="content">'+('投资额:'+nodeData[opts.investment] || '') + ('<br>左区业绩:'+nodeData[opts.nodeContent] || '') +('<br>右区业绩:'+nodeData[opts.right_market] || '')+('<br>对碰业绩:'+nodeData[opts.touch_market] || '')+ '</div>' : '');
