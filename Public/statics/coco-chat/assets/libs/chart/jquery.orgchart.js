  ...
  function createNode(nodeData, level, opts) {
   ...
   var url =  opts.url;
    ...
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + '>')
      ...
      .append(nodeData[opts.nodeContent] != '' ? '<div class="content">'+('投资额:'+nodeData[opts.investment] || '') + ('<br>左区业绩:'+nodeData[opts.nodeContent] || '') +('<br>右区业绩:'+nodeData[opts.right_market] || '')+('<br>对碰业绩:'+nodeData[opts.touch_market] || '')+'</div>' : '<div class="content">' + ('<p style="display: none">上级:'+'<span>'+nodeData[opts.parent_no]+'</span></p>' || '') +('<p style="display: none">位置:'+'<span>'+nodeData[opts.local]+'</span></p>此安置位置无人' || '')+'</div>');
    ...
    $nodeDiv.on('click', function(event) {
      if($(this).children('.title').html()=='点击注册')
      {
        //console.log(window.location.host);
        //return;
        var parent_no = $(this).find('span:first-child').html();
        var local = $(this).find('p:last-child>span').html();
        location.href=url+'?parent_no='+parent_no+'&local='+local;
      }
      $(this).closest('.orgchart').find('.focused').removeClass('focused');
      $(this).addClass('focused');
    });
